...
export default function StagePostsAdmin() {
  ...
  const load = (stageId) => {
    if (!stageId) return;
    api.get('/stage-posts', { params: { stageId } }).then((r) => setPosts(r.data));
  };
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      fd.append('stageId', String(editing.stageId || selectedStage));
    ...
    <div>
      ...
      <form onSubmit={submit} className="card p-6 mt-6 space-y-4 max-w-3xl">
        ...
        <div>
          ...
          <textarea rows={5} className="field" required value={editing.content} onChange={(e) => setEditing({ ...editing, content: e.target.value })} />
        </div>
        <div className="flex items-center gap-3">
          <input id="pp" type="checkbox" checked={editing.published} onChange={(e) => setEditing({ ...editing, published: e.target.checked })} />
          <label htmlFor="pp" className="text-sm text-white/80">Publicada</label>
        </div>
        <div>
      ...
      <div className="mt-8 space-y-3">
        {posts.map((p) => (
          <div key={p.id} className="card p-4 flex items-center gap-4">
            {p.image ? <img src={asset(p.image)} alt="" className="h-14 w-20 rounded-lg object-cover" /> : <div className="h-14 w-20 rounded-lg bg-white/5" />}
            <div className="flex-1 min-w-0">
              <div className="font-bold truncate">{p.title}</div>
              <div className="text-xs text-white/50">{new Date(p.createdAt).toLocaleString('es-AR')}</div>
            </div>
            <button onClick={() => setEditing({ ...p, stageId: String(p.stageId) })} className="btn-ghost text-sm">Editar</button>
            <button onClick={() => remove(p)} className="text-sm text-red-300 hover:text-red-200 px-3 py-2">Eliminar</button>
          </div>
        ))}
        {posts.length === 0 && <div className="text-sm text-white/50">Sin publicaciones en esta etapa.</div>}
      </div>
    </div>
  );
}
